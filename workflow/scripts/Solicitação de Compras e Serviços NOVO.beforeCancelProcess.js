function beforeCancelProcess(colleagueId,processId){
    ...
    if (movimentos != "") {
        ...
        for (var i = 0; i < movimentos.length; i++) {
            if (movimentos[i].IDMOV) {
                cancelaMovimentos(colleagueId, movimentos[i].IDMOV);
            }            
        }
    }
}

function cancelaMovimentos(colleagueId, RMidmov) {
    var hoje = new Date();
    ...
    var retorno = DatasetFactory.getDataset("CancelaMovimentoRM", null, [
        ...
        DatasetFactory.createConstraint("pUsuarioRM", colleagueId, colleagueId, ConstraintType.MUST),
